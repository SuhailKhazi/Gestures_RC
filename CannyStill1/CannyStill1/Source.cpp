...
int main() {
	...
	Serial * Arduino = new Serial("\\\\.\\COM17");
	...
	while (charCheckForEscKey != 27 && capWebcam.isOpened()) {		// until the Esc key is pressed or webcam connection is lost
		...
		for (int i = 0; i < v3fCircles.size(); i++) {		// for each circle . . .
			...
			if ((v3fCircles[i][0] > 400.0) /*&& (v3fCircles[i][1] >= 200.0) && (v3fCircles[i][1] <= 300.0)*/){//left
				word[0] = 'a';
				Arduino->WriteData(word, sizeof(word) - 1);
				Sleep(500);
			}

		  if ((v3fCircles[i][0] < 200.0)/*&& (v3fCircles[i][1] >= 200.0) && (v3fCircles[i][1] <= 300.0)*/) {//right
				word[0] = 'b';
				Arduino->WriteData(word, sizeof(word) - 1);
				Sleep(500);
			}
			
			if ((v3fCircles[i][1] < 200.0)/*&&(v3fCircles[i][0] >= 200.0) && (v3fCircles[i][0] <= 400.0)*/ ){//down
				word[0] = 'c';
				Arduino->WriteData(word, sizeof(word) - 1);
				Sleep(500);
			}
			if ((v3fCircles[i][1] > 300.0)/* && (v3fCircles[i][0] >= 200.0) && (v3fCircles[i][0] <= 400.0)*/) {//up
				word[0] = 'd';
				Arduino->WriteData(word, sizeof(word) - 1);
				Sleep(500);
			}
			//if (v3fCircles[i][0] < 150.0)/*&& (v3fCircles[i][1] >= 200.0) && (v3fCircles[i][1] <= 300.0)*/ {//stop
			//	word[0] = 'e';
			//	Arduino->WriteData(word, sizeof(word) - 1);
			//	Sleep(500);
			//}
